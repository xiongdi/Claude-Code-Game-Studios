...
3. 配置设置：
   - **Agent Radius**：agent 有多宽（默认 0.5m）
   - **Agent Height**：agent 有多高（默认 2m）
...
// 停止距离
agent.stoppingDistance = 2f; // 到达目的地前 2m 停止
...
// Carve：在 NavMesh 中创建洞（agent 回避）
// Don't Carve：agent 推开（局部回避）
```
...
- **NavMesh 瓦片**：对大型世界使用平铺 NavMesh（NavMeshComponents 包）
