...
### 选择正确的渲染管线
- **URP (Universal)**：移动端、跨平台、性能良好 ✅ 推荐大多数游戏使用
...
### 使用 Input System 包（而非 Legacy Input）
...
### 使用 RenderGraph API 进行自定义 Pass（URP/HDRP）
...
```csharp
// ✅ GPU Instancing（数千个对象，最少 draw call）
...
### 使用 NativeContainers（Jobs 中不用托管数组）
...
**替代：** UNet（已弃用）、MLAPI（重命名为 Netcode for GameObjects）。
...
| **Jobs** | Burst + IJobParallelFor | 繁重工作的 Coroutines |
